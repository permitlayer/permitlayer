...
- **End-user authentication** uses per-user bearer tokens minted by `agentsso agent register --name <name>` and written to `~/.agentsso/agent-bearer.token` (mode 0600, owned by the invoking user). The daemon writes the token using a `tmp + chown + atomic-rename + O_NOFOLLOW` pattern that defends against CWE-367 / filelock-CVE-2026-22701 class of TOCTOU symlink attacks.
...
- **Plugin escape becomes root RCE.** rc.22 ships plugins in-process inside the root LaunchDaemon. Customer-installed plugins are disabled by default (`daemon.toml` `plugin_loader.allow_user_plugins: false`); built-in connectors (gmail / calendar / drive) are code we control + audit. A post-rc.22 story lands OS-level sandboxing (`sandbox_init` profile or subprocess-with-setuid jail) before rc.23 ships.
