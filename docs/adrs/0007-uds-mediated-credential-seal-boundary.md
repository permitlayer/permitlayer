...
The CLI's `connect.rs` opened nine fs/keystore touchpoints — agent-store lookup (`resolve_agent_policy_name`), credential meta stat + parse, vault-dir writability check (`check_vault_dir_writable`), keystore + master-key access (`compute_active_key_id`), the seal flow (`CredentialFsStore::new` + `vault.seal` + `store.put` + `write_metadata_atomic`), the policy scope-merge flow (`add_scopes_to_policy`), the post-edit reload kick (`post_reload`), and the per-retry verify unseal (`read_access_token` + `verify::verify_connection`). Each one required either (a) the CLI to run as root or (b) the operator's UID to own the relevant fs node. ADR-0006's privilege model says only root owns these paths. Both can't be true; the symptom was the connect flow being root-only by accident. (Round-3 review P73: ADR cites symbol names rather than line numbers — line numbers in `connect.rs` rot on every refactor; symbols stay stable across the lifetime of this ADR, matching the same policy applied to zeroize crate refs below.)
...
- **Browser-in-daemon (the daemon owns the OAuth flow)**. Rejected: the daemon has no GUI session at all. macOS `LaunchDaemons` run in the system context with no Aqua session bus attached — `open::that()` from the daemon process couldn't reach a GUI under any condition (this is unconditional and unrelated to the SSH-CLI failure mode the post-7.30 CLI heuristic now avoids on its own side). The daemon could in principle drive a headless OAuth flow, but the redirect-URL paste requires an operator's terminal, which the daemon doesn't have either. (Round-3 review P74: rephrased — the prior wording "same failure as the SSH operator's CLI" became stale once the CLI gained the `should_skip_browser_open` heuristic; the actual rejection rationale is the daemon's GUI-sessionlessness, not parity with the CLI's pre-7.30 failure mode.)
