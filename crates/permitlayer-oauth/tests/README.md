# Integration tests for `permitlayer-oauth`
...
Old idiom (one binary per file): `cargo test --test oauth_flow -- my_test`.

New idiom:
```sh
cargo test --test integration oauth_flow::my_test
cargo nextest run -p permitlayer-oauth -E 'test(my_test)'
```

nextest's `test()` filter matches on the full module path (e.g.
`integration::oauth_flow::my_test`), so partial matches still work.
