...
/// Run the Phase A→G rotation. Caller (`agentsso rotate-key`) has
/// already verified the daemon is stopped, brew-services is not
/// managing, the keystore is native, and the confirmation prompt
/// passed (or `--yes`).
///
/// On success, returns the freshly-minted master key bytes. The
/// `agentsso rotate-key` CLI caller binds to `_` and discards
/// (rotate-key is a CLI; the daemon is stopped).
pub(crate) async fn run_rotation(
    ...
    let (old_key_bytes, new_key_bytes, old_key_id, new_key_id, active_marker) = match resolved {
        ...
        None => {
            ...
            drop(vault_lock);
            // A prior rotation's Phase F committed but the marker
            // delete crashed; the daemon's master key in the keystore
            // is already the new key. Re-fetch it so we can return
            // the current key bytes to the caller. The CLI caller
            // discards them (daemon is stopped during rotate-key) but
            // the function signature must still satisfy `Result<...>`.
            match keystore.master_key().await {
                ...
                Err(e) => {
                    // Deep edge case: keystore unavailable AND a prior
                    // rotation was committed. The current binary
                    // cannot read the post-rotation key — operator
    ...
    // Clone the new key bytes before consuming them in `Vault::new`
    // so we can return them to the caller after Phase G; the CLI
    // caller discards them since the daemon is stopped during
    // rotate-key. Clone of `Zeroizing<[u8; 32]>` is a 32-byte copy.
    ...
    // (crashed) attempt — skip. Otherwise rebuild. Each agent's bearer
    // token is reminted (`agt_v2_*`) and persisted via
    ...
    for agent in agents {
        ...
        // Mint a fresh v2 token for each agent.
    ...
    // operator can `cat` it after the retry succeeds. Skip the file
    // write when the rotation produced no tokens (empty vec — happens
    // if there are zero agents registered).
    ...
    // Hand the new master key bytes back to the caller. `rotate-key`
    // discards them because the daemon is stopped during rotation by
    // precondition.
